...
def enable_site():
    ...
    with app.app_context():
        ...
        if config:
            print(
                "La configuración 'MAIN_SITE_ENABLED' ya existe. Actualizando valor a '1'."
            )
            config.valor = "1"
        else:
            print(
                "La configuración 'MAIN_SITE_ENABLED' no existe. Creándola con valor '1'."
            )
            config = ConfiguracionSistema(
                clave="MAIN_SITE_ENABLED",
                valor="1",
                descripcion="Habilita o deshabilita el sitio principal.",
            )
