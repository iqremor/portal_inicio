...
- **Vista de Selección**: Implementar `frontend/pages/simulacro.html` para desglosar las áreas (Matemáticas, Sociales, etc.).
- **Visualización de Progreso**: Integrar en tiempo real las notas obtenidas en cada componente dentro del panel de "Nota por componente".
- **Flujo de Retorno**: Configurar el sistema para que al finalizar un examen, el usuario regrese al Lobby del simulacro con su nota actualizada, en lugar de ir al Dashboard general.

---

## ⚙️ Fase 3: Control Administrativo Dinámico

_Objetivo: Permitir al administrador habilitar o deshabilitar módulos completos._

- **Backend (Configuración)**: Añadir flags de visibilidad en la base de datos para `MODULE_PREICFES_ENABLED`, `MODULE_PREUNAL_ENABLED`, etc.
- **Panel Admin**: Crear interruptores (toggles) en la sección de "Ajustes Globales" para controlar qué paneles ven los estudiantes.
- **Acceso por Grado**: Refinar la lógica para que ciertos módulos (como Preunal) sean visibles solo para grados específicos (ej. 11°).

---

## 📊 Fase 4: API de Resumen de Desempeño

_Objetivo: Servir los datos necesarios para el panel de puntajes del simulacro._

- **Endpoint de Resumen**: Crear `/api/usuario/<codigo>/resumen_notas` que agrupe las mejores notas de cada área del último simulacro activo.
- **Integración Frontend**: Sincronizar los valores de "0.0" en el panel de puntajes con datos reales del estudiante.
