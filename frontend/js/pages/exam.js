...
class Exam {
  ...
  render() {
    ...
    document.getElementById('examArea').textContent = this.areaId
      .replace('_', ' ')
      .toUpperCase();
    this.renderQuestion();
  ...
  renderQuestion() {
    ...
    document.getElementById('questionPoints').innerHTML =
      `<i class="fas fa-star"></i><span>${question.puntos} puntos</span>`;
    document.getElementById('questionText').textContent = question.pregunta;

    const progress =
      ((this.currentQuestion + 1) / this.examData.preguntas.length) * 100;
    document.getElementById('progressFill').style.width = `${progress}%`;
    document.getElementById('progressText').textContent = `Pregunta ${
      this.currentQuestion + 1
    } de ${this.examData.preguntas.length}`;
  ...
  renderOptions(question) {
    ...
    question.opciones.forEach((opcion, index) => {
      ...
      optionDiv.innerHTML = `<input type="radio" name="question_${
  ...
  updateNavigation() {
    ...
    document.getElementById('btnNext').disabled =
      this.currentQuestion === this.examData.preguntas.length - 1;
  }
  ...
  startTimer() {
    ...
    this.timer = new Timer(
      ...
      () => {
        showNotification(
          '¡Tiempo agotado! El examen se enviará automáticamente.',
          'error'
        );
        this.finishExam();
      }
  ...
  setupEventListeners() {
    ...
    document
      .getElementById('btnFinishExam')
      .addEventListener('click', () => this.showFinishModal());
  ...
  showFinishModal() {
    ...
    document
      ...
      .addEventListener('click', () => this.hideFinishModal());
    document
  ...
  async finishExam() {
    ...
    try {
      ...
      if (!userCodigo) {
        showNotification(
          'Error: Código de usuario no encontrado para finalizar el examen.',
          'error'
        );
        return;
      }
